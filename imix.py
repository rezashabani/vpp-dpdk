...
class STLImix(object):
    ...
    def create_stream(self, size, pps, isg, vm):
        ...
        payload_size = size - len(base_pkt)
        pad = max(0, payload_size) * 'x'  # Pad with 'x' to adjust size

        # Generate payload with 8 bytes of '1's
        payload = b'\x01' * min(payload_size, 8)  # Ensure payload does not exceed 8 bytes

        # Build the packet with payload
        pkt = STLPktBuilder(pkt=base_pkt/payload,
                            vm=vm)
    ...
    def get_streams(self, direction, tunables, **kwargs):
        parser = argparse.ArgumentParser(description=f'Argparser for {os.path.basename(__file__)}',
...
def register():
    return STLImix()
root@reza-VirtualBox:~# 
root@reza-VirtualBox:~# 
root@reza-VirtualBox:~# 
root@reza-VirtualBox:~# 
root@reza-VirtualBox:~# nano v3.04/stl/imix.py 
root@reza-VirtualBox:~# 
root@reza-VirtualBox:~# cat v3.04/stl/imix.py 
from trex_stl_lib.api import *
import argparse
import os
